...
class AppDelegate: NSObject,
    ...
    /// Drives the Settings window's live appearance overrides. Also used to (re)apply any
    /// persisted overrides immediately at launch, before the Settings window is ever shown.
    ...
    override init() {
        ...
        ghostty = Ghostty.App()
#endif
        settingsModel = SettingsModel(ghostty: ghostty)
        super.init()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Apply any persisted live Settings overrides now that Ghostty is ready.
    ...
    func application(_ sender: NSApplication, openFile filename: String) -> Bool {
        ...
        if requiresConfirm {
            ...
            let alert = NSAlert()
            alert.messageText = "Allow Wisp to execute \"\(filename)\"?"
    ...
    /// Toggles the Warp-style file-explorer sidebar in the focused terminal window.
    @IBAction func toggleFileSidebar(_ sender: Any?) {
        NotificationCenter.default.post(name: .wispToggleFileSidebar, object: nil)
    }

    /// Adds a "Toggle File Sidebar" item (⌘⌥B) to the View menu at runtime, kept in code
    /// alongside its action rather than in the xib.
    private func installFileSidebarMenuItem() {
        guard let viewMenu = NSApp.mainMenu?.item(withTitle: "View")?.submenu else { return }
        let item = NSMenuItem(
            title: "Toggle File Sidebar",
            action: #selector(toggleFileSidebar(_:)),
            keyEquivalent: "b")
