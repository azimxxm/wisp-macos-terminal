...
#error("unsupported")
        ...
        /// The configured font size, in points. Note the underlying config value is a
        /// 32-bit float so we must read it into a `Float`, not `Double`.
        var fontSize: Double {
        ...
        /// The background blur radius, in points. 0 means no blur. macOS "glass" styles
        /// (negative C values) surface as 0 here since they aren't a plain numeric radius.
        var backgroundBlurRadius: Double {
            guard let config = self.config else { return 0 }
            var v: Int16 = 0
            let key = "background-blur"
            _ = ghostty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8)))
            return v > 0 ? Double(v) : 0
        }
