...
extension Ghostty {
    ...
    class App: ObservableObject {
        ...
        /// Apply a set of live configuration overrides without touching the user's on-disk
        /// configuration file. This starts from the same base configuration `reloadConfig`
        /// uses (our configured path, or the default files), layers the given `key = value`
        /// overrides on top via a scratch file (there is no load-from-string API), and
        /// applies the result to the running app immediately.
        ///
        /// Pass an empty `overrides` dictionary to revert back to the base configuration.
        /// This must be called on the main thread since it mutates global Ghostty app state.
        func applySettings(overrides: [String: String]) {
#if os(macOS)
            guard let app = self.app else { return }
            guard let cfg = ghostty_config_new() else {
                Ghostty.logger.warning("failed to allocate configuration for applySettings")
                return
            }
            ...
            // There is no "load from string" API so we write our overrides to a scratch
            // file and load that as an overlay on top of the base configuration.
            var overrideFile: URL?
            if !overrides.isEmpty {
                let contents = overrides.map { "\($0.key) = \($0.value)" }.joined(separator: "\n")
                let file = FileManager.default.temporaryDirectory
                    .appendingPathComponent("ghostty-live-settings-\(UUID().uuidString).conf")
                do {
                    try contents.write(to: file, atomically: true, encoding: .utf8)
                    overrideFile = file
                    ghostty_config_load_file(cfg, file.path)
                } catch {
                    Ghostty.logger.warning("failed to write live settings overrides: \(error, privacy: .public)")
                }
            }
            ...
            /// applied config will be updated in ``Self.configChange(_:target:v:)``
#else
            Ghostty.logger.warning("applySettings is not supported on this platform")
#endif
        }
