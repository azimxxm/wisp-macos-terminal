...
extension Ghostty {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            // Warp-style: ⌘-click a file path printed in the output (e.g. from `ls`) opens
            // it in Wisp's side pane. We consume the whole click (press + the matching
            // release) so it never starts a selection or reaches the child app. If the word
            // under the cursor isn't an existing path, this is a no-op and we fall through
            // to normal handling — including Ghostty's own ⌘-click-to-open for URLs.
            if event.modifierFlags.contains(.command),
               event.clickCount == 1,
               commandClickOpenPath() {
                suppressNextLeftMouseUp = true
                return
            }
        ...
        /// The existing file/dir the mouse is currently over, if the word under the cursor
        /// resolves to one (relative to the surface's cwd). Powers both ⌘-click and the
        /// right-click "Open / Show in Finder / Copy Path" menu items.
        private func resolvedFilePathUnderCursor() -> URL? {
            guard let surface = self.surface else { return nil }

            var text = ghostty_text_s()
            guard ghostty_surface_quicklook_word(surface, &text) else { return nil }
            defer { ghostty_surface_free_text(surface, &text) }
            guard text.text_len > 0, let cString = text.text else { return nil }

            let cwd = pwd.flatMap { URL(fileURLWithPath: $0) }
            return TerminalFilePath.resolve(token: String(cString: cString), cwd: cwd)
        }

        /// Opens the path under the cursor in the owning window, if there is one. Returns
        /// whether it handled the click.
        private func commandClickOpenPath() -> Bool {
            guard let url = resolvedFilePathUnderCursor() else { return false }
            openResolvedPath(url)
            return true
        }

        /// Routes a resolved path to the window that owns this surface via a notification
        /// the focused `TerminalView` filters on.
        private func openResolvedPath(_ url: URL) {
            NotificationCenter.default.post(
                name: .wispOpenPath,
                object: self,
                userInfo: [wispOpenPathURLKey: url])
        }
        ...
        override func menu(for event: NSEvent) -> NSMenu? {
            ...
            // Warp-style: if the right-click landed on a file path in the output, offer to
            // open it in Wisp, reveal it in Finder, or copy its path.
            if let pathURL = resolvedFilePathUnderCursor() {
                item = menu.addItem(
                    withTitle: "Open “\(pathURL.lastPathComponent)” in Wisp",
                    action: #selector(wispOpenPathInPane(_:)),
                    keyEquivalent: "")
                item.representedObject = pathURL
                item.setImageIfDesired(systemSymbolName: "doc.text")

                item = menu.addItem(withTitle: "Show in Finder", action: #selector(wispRevealPathInFinder(_:)), keyEquivalent: "")
                item.representedObject = pathURL
                item.setImageIfDesired(systemSymbolName: "folder")

                item = menu.addItem(withTitle: "Copy Path", action: #selector(wispCopyPath(_:)), keyEquivalent: "")
                item.representedObject = pathURL
                item.setImageIfDesired(systemSymbolName: "doc.on.clipboard")
        ...
        @objc private func wispCopyPath(_ sender: NSMenuItem) {
            guard let url = sender.representedObject as? URL else { return }
